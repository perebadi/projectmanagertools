...
public interface ProjectTaskRepository   extends JpaRepository<Task, Serializable>{
	...
	@Query(value = "SELECT id, projectname, user_username, projectactive, backlog, todo, inprogress, done FROM view_project_task WHERE  user_username = ?1 or id in (SELECT project_id FROM projectmanagertools.project_user WHERE user_username = ?1);", nativeQuery = true)	
